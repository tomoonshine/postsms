...
class catalog extends def_module {
	...
	/* Выводит список подразделов в указанном разделе каталога */
	public function getCategoryList($template = "default", $category_id = false, $limit = false, $ignore_paging = false, $i_need_deep = 0) {
		// первый параметр Шаблон, по которому выводятся подразделы. В XSLT-шаблонизаторе игнорируется
		if(!$template) $template = "default";
		
		// Присвоить переменным из списка значения
		// Получаем блоки шаблонов "category_block", "category_block_empty", "category_block_line" из файла шаблона $template, из папки "catalog/.$template ".
		...
		// Глубина поиска объектов по умолчанию в текущем
		...
		// идентификатор или путь до раздела каталога
		...
		// Игнорировать переход по страницам
		if($ignore_paging) $curr_page = 0;

		// назначить выборку
		$sel = new umiSelection;
		// отбор заданных элементов
		$sel->addElementType($hierarchy_type_id);
		// начиная с заданного раздела и глубиной 
		$sel->addHierarchyFilter($category_id, $i_need_deep);
		// количество объектов и номер страгницы
		$sel->addLimit($per_page, $curr_page);

		$result = umiSelectionsParser::runSelection($sel);
		$total = umiSelectionsParser::runSelectionCounts($sel);

		// Если в выборке присутствуют элементы
		if(($sz = sizeof($result)) > 0) {

			// массив с общей информацией по выборке
			$block_arr = array();
			// массив выбранных элементов
			$lines = array();

			for($i = 0; $i < $sz; $i++) {
				// пока не достигнут лимит
				if ($i < $limit || $limit == false) {
					// получить id элемента из результата
					$element_id = $result[$i];
					...
					// неопределён элемент -> следующий цикл
					if(!$element) continue;
					...
					// построить результат по шаблону
